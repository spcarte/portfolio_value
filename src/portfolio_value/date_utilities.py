...
def interpolate_time_value(original_dates, original_value, want_dates, interpolation_type='linear'):
    ...
    want_dates_between = np.cumsum(np.insert(np.array([(want_dates[ii+1]-want_dates[ii]).days for ii in range(len(want_dates)-1)]), 0, 0))+original_dates_between[find_nearest_date_index(want_dates[0], original_dates)-1]
